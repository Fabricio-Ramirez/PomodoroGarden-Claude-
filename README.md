...
GitHub Actions also builds the app on Windows for every push and runs the tests. You can download the `.exe` from the run's **Artifacts** section on the Actions tab. To publish a release, push a version tag (for example `git tag v2.1.0 && git push origin v2.1.0`) and the workflow attaches the `.exe` to a new GitHub Release.
